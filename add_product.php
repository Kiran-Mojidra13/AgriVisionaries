...
$farmer_id = $_SESSION['farmer_data']['id'];  // Assuming session has farmer data and 'id' is stored
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($_FILES['product_image']['name']) {
        ...
        if (move_uploaded_file($image_tmp, $image_path)) {
            // Insert the product data into the database, including farmer_id
            $insert_query = "INSERT INTO products (category_id, sub_category_id, product_name, product_image, product_price, stock, farmer_id) 
                             VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($insert_query);
            $stmt->bind_param("iissdii", $category_id, $sub_category_id, $product_name, $image_name, $product_price, $stock, $farmer_id);
            $stmt->execute();
